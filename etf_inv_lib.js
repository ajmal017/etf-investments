  ...
  const BASE_URL = 'https://api.iextrading.com/1.0/tops';
...
function updateDataForBatch(symbols, addTitle) {
  ...
  fetch(url).then(response => response.json()).then(json => {
    json.forEach(data => {
      ...
      let formattedPrice = formatQuote(data.lastSalePrice);
      let latestVolume = data.volume/data.marketPercent;
      let formattedLatestVolume = formatNumberToWords(latestVolume);

      let volProjection = (latestVolume/latestVolume)/vwap_curve[curTime];
      ...
      document.querySelectorAll(`[data-symbol="${symbol}"] .stock-price`).forEach(e => {
        ...
        e.originalNumeric = data.lastSalePrice;
      ...
      document.querySelectorAll(`[data-symbol="${symbol}"] .stock-latest-volume`).forEach(e => {
        ...
        e.originalNumeric = latestVolume;
        e.setAttribute('style', `background-color: rgba(${rgbColor}, ${rgbOpacity})`);
      });
